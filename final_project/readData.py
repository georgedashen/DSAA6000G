...
def cacu_features(df:pd.DataFrame, window:int, step:int,tag:str,eqData:pd.DataFrame,flag:str) ->pd.DataFrame:
    ...
    df[averageName] = df[averageName]-df[averageName].mean()
    df['diff_1'] = df.groupby('StationID')[averageName].shift(1)
    df['diff_1'] = df[averageName].values - df['diff_1'].values
...
if __name__ == "__main__":
    ...
    for i,area in enumerate(area_groups):
        ...
        for flag in ['train', 'valid']:
            ...
            #_magn_res.to_csv(f'./area_feature/area_{i}_{flag}_magn.csv')
            _sound_res.to_csv(f'./area_feature/area_{i}_{flag}_sound.csv')
