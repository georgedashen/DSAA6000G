...
MODEL_NAME = "BalanceweightedPrecisionEval200"  
def optimize(config):
    ...
    weighted_f1 = 0.1*f1_array[0] + 5*f1_array[1] + 20*f1_array[2] + 10*f1_array[3] + 20*f1_array[4]
    ...
    weighted_precision = 0.1*precision_array[0] + 5*precision_array[1] + 10*precision_array[2] + 30*precision_array[3] + 10*precision_array[4]
    #weighted_precision = 1*precision_array[0] + 10*precision_array[1] + 5*precision_array[2] + 20*precision_array[3] + 15*precision_array[4]
    weighted_recall = 0.1*recall_array[1] + 5*recall_array[1] + 30*recall_array[2] + 30*recall_array[3] + 30*recall_array[4]
...
if __name__ == "__main__":
    ...
    printOverall = True
    if(printOverall): 
        overall = open(f'./evaluation/overall_sm_{MODEL_NAME}.txt','w+')
        overall_M = pd.Series()
    ...
    #训练数据
        ...
        balanceM = True
        train_data['class'] = 0
        for i, ss in enumerate(train_data['label_M']):
            if(ss<3.5):
                train_data['class'].iloc[i] = 0
            elif(ss<4.0):
                train_data['class'].iloc[i] = 1
            elif(ss<4.5):
                train_data['class'].iloc[i] = 2
            elif(ss<5.0):
                train_data['class'].iloc[i] = 3
            else:
                train_data['class'].iloc[i] = 4
        ...
        if(balanceM):
            ...
            num = short_data['class'].value_counts()
            n_class = len(num)
            N = round(len(long_data)/n_class)
            for i in range(n_class):
                new_data = pd.concat([new_data, short_data[short_data['class']==num.index[i]].sample(N, replace=True)]) #whether keep all class the same?
            short_data = new_data.copy(deep=True)
            del new_data
        else:
        ...
        train_feature = train_data.drop(['label_M','label_long','label_lati','Day','class'],axis=1)
    ...
    #验证数据
        ...
        spaces = {
            ...
            "num_leaves":hp.choice("num_leaves",range(5,43)),
        ...
        cpf = open('./evaluation/'+str(area)+f'_sm_{MODEL_NAME}.txt','w+')
